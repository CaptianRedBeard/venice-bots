...
class AgentShell:
    ...
    This agent is a "doer" that receives a dictionary of instantiated tool objects and a persona
    configuration from a Conductor and uses them to perform tasks.
    """
    def __init__(
        self,
        persona_name: str,
        ...
        agent_acl: Optional[Dict[str, str]] = None
    ):
        """
        Initializes the Agent Shell.
        Args:
            ...
            context_manager (Optional[Any]): An instance of the ContextManager for memory retrieval.
            agent_acl (Optional[Dict[str, str]]): The agent's access control list for filtering memory.
        """
        # 1. Store Persona Configuration directly
        self.persona_name = persona_name
        self.persona_config = persona_config
        # 2. Store context-related attributes
        self.context_manager = context_manager
        self.agent_acl = agent_acl or {'access_level': 'public'}
        ...
        # 4. Store tools using their function name for easy lookup
        self.tools = {}
    ...
    def _setup_logging(self):
        """ Configures the agent's logger to write to a file by default. This prevents cluttering the terminal during normal operation. """
    ...
    def _call_llm_with_functions(self, user_input: str, openai_schemas: List[Dict[str, Any]]) -> tuple:
        """ Performs a standard OpenAI function-calling API call. This is a generic helper that can be used by any agent. """
    ...
    def process_prompt(self, user_input: str) -> str:
        """
        Processes a user prompt by first injecting context from the ContextManager,
        then using the standard tool-calling flow.
        """
        self.logger.info("prompt_received", extra={"details": {"prompt": user_input}})
        
        # Pre-fetch and inject context
        full_prompt = user_input
        if self.context_manager:
            retrieved_context = self.context_manager.get_context(user_input, self.agent_acl)
            # Augment the prompt with the retrieved context
            full_prompt = f"""You are an assistant that answers questions based ONLY on the provided context. If the context does not contain the answer, state that you could not find the information.
...
Answer:"""
            # print(f"DEBUG: Full augmented prompt being sent to LLM:\n---\n{full_prompt}\n---") # <--- ADD THIS FINAL DEBUG PRINT
        ...
        try:
            ...
            # Use the context-augmented prompt for the LLM call
            function_name, arguments = self._call_llm_with_functions(full_prompt, openai_schemas)
